...
var extendApply = function (obj, objects, fn) {
    ...
    _.each(objects, function (currentObject, objectIndex) {
        if (currentObject) {
            _.each(_.keys(currentObject), function (key, valueIndex) {
...
var deepExtend = function (obj, objects, options) {
    options = options || {};
    _.each(objects, function (currentObject) {
        if (currentObject) {
            _.each(_.keys(currentObject), function (key) {
                if ((_.isArray(obj[key]) || !obj[key]) && _.isArray(currentObject[key])) {
                    if (options.replace) {
                        obj[key] = deepExtend(obj[key] || [], [currentObject[key]]);
                    } else {
                        obj[key] = (obj[key] || []).concat(currentObject[key].slice());
                    }
